...
namespace middlerApp.IDP.Library.Validators
{
    /// <summary>
    /// Validates secrets using the registered validators
    /// </summary>
    public class CustomSecretValidator : ISecretsListValidator
    {
        ...
        public CustomSecretValidator(ISystemClock clock, IEnumerable<ISecretValidator> validators, ILogger<ISecretsListValidator> logger)
        {
            _clock = clock;
        ...
        public async Task<SecretValidationResult> ValidateAsync(IEnumerable<Secret> secrets, ParsedSecret parsedSecret)
        {
            ...
            var expiredSecrets = secretsArray.Where(s => s.Expiration.HasValue && s.Expiration.Value < DateTime.Now).ToList();
            ...
            var currentSecrets = secretsArray.Where(s => !s.Expiration.HasValue || s.Expiration.Value < DateTime.Now).ToArray();
